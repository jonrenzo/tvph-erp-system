        ...
        {stats.map((stat, i) => (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <span
                className={`text-[10px] font-bold uppercase tracking-widest ${stat.trendColor}`}
              >
                {stat.trend}
              </span>
            </div>
        ...
        <div className="lg:col-span-2 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#071F15] overflow-hidden shadow-sm">
          <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <h2 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <History className="h-5 w-5 text-primary" /> Recent System
              Activity
            </h2>
            <Link
          ...
          <div className="divide-y divide-slate-50 dark:divide-slate-800/50">
            {recentLogs?.length === 0 ? (
              ...
              recentLogs?.map((log: any) => (
                <div
                  key={log.id}
                  className="p-4 hover:bg-slate-50/50 dark:hover:bg-slate-800/10 transition-colors flex items-start gap-4"
                >
                  <div
          ...
          <div className="rounded-2xl bg-primary p-6 text-white shadow-lg shadow-primary/20 relative overflow-hidden">
            <div className="relative z-10">
              ...
              <p className="text-xs text-white/80 mt-2 leading-relaxed">
                ...
                waiting for accreditation review and{" "}
                <span className="font-bold underline">
            ...
            <div className="space-y-2">
              {[
                ...
                <Link
                  key={i}
                  href={link.href}
                  className="flex items-center justify-between p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 group transition-colors"
                >
                  <span className="text-xs text-slate-600 dark:text-slate-400 group-hover:text-primary transition-colors">
                    {link.label}
                  </span>
    ...
    <div className="space-y-8 animate-pulse">
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {[...Array(4)].map((_, i) => (
          <div
            key={i}
            className="h-32 rounded-2xl bg-slate-100 dark:bg-slate-800/50"
          />
        ))}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
