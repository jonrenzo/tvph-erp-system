...
export async function createPaymentRequest(
  ...
  if ((po as { source?: string }).source !== 'legacy') {
    ...
    const satisfiedDocTypes = new Set(
      (vendorDocs || []).filter((d) => d.status === 'submitted' || d.status === 'approved').map((d) => d.doc_type),
    );
    const missingRequiredDocs = PAYMENT_REQUIRED_DOC_TYPES.filter((t) => !satisfiedDocTypes.has(t));
    if (missingRequiredDocs.length > 0) {
      return {
        error: `Blocked: required accreditation document${missingRequiredDocs.length > 1 ? 's' : ''} ${missingRequiredDocs.map((t) => docTypeLabel(t)).join(', ')} ${missingRequiredDocs.length > 1 ? 'are' : 'is'} missing or not submitted. Complete vendor accreditation before sending a payment request.`,
      };
    }
  }
  ...
  if (completionCertId) {
    const { data: cert } = await supabase.from('po_completion_certificates').select('percent_complete, status').eq('id', completionCertId).single();
    ...
    if (amount > ceiling) {
      return { error: `Requested amount (₱${amount.toLocaleString()}) exceeds the completion certificate ceiling (${percentComplete}% = ₱${ceiling.toLocaleString()}).` };
    }
  } else if (amount > Number(po.amount)) {
  ...
  defer(async () => {
    ...
    await sendPaymentRequestNotification(poId, po.vendor_id, amount, dueInDays ?? 30, notes || null, user.id, po.po_number as string, vendorName);
  });
