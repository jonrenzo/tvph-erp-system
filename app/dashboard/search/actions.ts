...
export async function globalSearch(query: string) {
  if (!query || query.length < 2) return { vendors: [], pos: [], invoices: [], projects: [], payments: [], documents: [] };

  const supabase = await createClient();
  const safeQuery = `%${query}%`;

  const [vendorsRes, posRes, invoicesRes, projectsRes, paymentsRes, documentsRes] = await Promise.all([
