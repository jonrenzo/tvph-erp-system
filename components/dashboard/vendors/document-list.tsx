...
import { uploadDocument, approveVendorDocument } from "@/app/dashboard/vendors/actions";
...
export function DocumentList({ vendorId, documents, userRole }: { vendorId: string, documents: Document[], userRole?: string }) {
  ...
  // Check NDA status for header indicator
      ...
      <div className={`flex items-center gap-3 p-4 rounded-2xl border ${
        ...
        <div>
          <p className={`text-sm font-semibold ${
            ndaApproved ? 'text-emerald-700 dark:text-emerald-400' 
            : ndaExpired ? 'text-red-700 dark:text-red-400' 
            : 'text-amber-700 dark:text-amber-400'
          }`}>
            {ndaApproved ? 'Signed NDA Approved' : ndaExpired ? 'Signed NDA Expired' : 'Signed NDA Not Approved'}
          </p>
          <p className={`text-xs ${
            ndaApproved ? 'text-emerald-600/80 dark:text-emerald-400/60' 
            : ndaExpired ? 'text-red-600/80 dark:text-red-400/60'
                ...
                <tr key={type.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/10 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {/* Approve button — admin only, for submitted docs */}
                      {canApprove && (
