...
import { Save, FileText, Upload, Link as LinkIcon, Tag, AlertTriangle, Info } from "lucide-react";
...
export function CreateInvoiceForm({ vendors, pos, defaultPoId }: { vendors: Vendor[], pos: PO[], defaultPoId?: string }) {
  const [state, formAction, isPending] = useActionState(createInvoice, null);
  const [, startDiscard] = useTransition();

  const initialPo = defaultPoId || "";
  const initialVendor = initialPo ? (pos.find(p => p.id === initialPo)?.vendor_id || "") : "";
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      {/* Invoice-driven PR: PR is auto-created on submit when a PO is linked — no PR selection needed */}
      {selectedPo && (
        <div className="flex items-center gap-2 p-3 rounded-xl bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800/50 text-blue-700 dark:text-blue-400 text-sm">
          <Info className="h-4 w-4 shrink-0" />
          A Payment Request for ₱{amount || '—'} will be auto-created and pending approval once this invoice is recorded.
        </div>
      )}
